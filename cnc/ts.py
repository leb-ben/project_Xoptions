...
# Perform Kraken API request to get current Ether price in USD
response = requests.get(KRAKEN_API_ENDPOINT, params={"pair": ASSET_PAIR})
if response.ok:
    result = response.json().get("result")
    if result and isinstance(result, dict) and ASSET_PAIR in result:
        ticker_info = result.get(ASSET_PAIR)
        if "c" in ticker_info:
